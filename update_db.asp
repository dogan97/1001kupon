...
' 2. Adim: Eski Proseduru Silme
Dim sqlDrop
sqlDrop = "IF OBJECT_ID('dbo.sp_kupon_tanim_ekle', 'P') IS NOT NULL DROP PROCEDURE dbo.sp_kupon_tanim_ekle"
conn.Execute sqlDrop

If Err.Number <> 0 Then
    Response.Write "<p style='color:orange'>Uyari (SP Silme): " & Err.Description & "</p>"
    Err.Clear
End If

' 3. Adim: Proseduru Yeniden Olusturma (CREATE PROCEDURE)
' DUZELTME: NEWID() -> VARCHAR(36) -> LEFT(..., 8)
Dim sqlCreate
sqlCreate = "CREATE PROCEDURE dbo.sp_kupon_tanim_ekle " & _
       ...
       "VALUES(@firma_id, @kullanici_id, @tanim_adi, @aciklama, @indirim_tipi, @indirim_degeri, @sepet_limit, @gecerlilik_gun, 'DEF-' + LEFT(CAST(NEWID() AS VARCHAR(36)), 8)); " & _
...
If Err.Number <> 0 Then
    Response.Write "<p style='color:red'>Hata (SP Olusturma): " & Err.Description & "</p>"
Else
